...
/*   Updated: 2024/02/21 15:32:37 by jblaye           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	render_bottom_margin(t_data *data)
{
	...
	i = 1;
	if (push_tile(data->mlx, data->assets[T_LOWBORD], 0, data->map.size[Y]) == 0
		|| push_tile(data->mlx, data->assets[T_LOWBORD], data->map.size[X] - 1,
			data->map.size[Y]) == 0)
		return (0);
	while (i < data->map.size[X] - 1)
	{
		if (push_tile(data->mlx, data->assets[T_LOWBORD], i, data->map.size[Y])
			== 0)
			return (0);
		i++;
	}
...
int	render_corners(t_data *data)
{
	...
	if (push_tile(data->mlx, data->assets[T_TL], 0, 0) == 0
		|| push_tile(data->mlx, data->assets[T_TR], x, 0) == 0
		|| push_tile(data->mlx, data->assets[T_BL], 0, y) == 0
		|| push_tile(data->mlx, data->assets[T_BR], x, y) == 0)
		return (0);
	return (1);
}
